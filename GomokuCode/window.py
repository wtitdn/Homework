...
class GomokuWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        palette = QPalette()
        self.button1 = QPushButton('战况',self)
    ...
    def clickButton(self):
        QMessageBox.about(self,'战绩','电脑胜利次数：'+str(self.ai)+'<br>玩家胜利次数：'+str(self.player))
    ...
    def end_flash(self):
        ...
        if self.flash_cnt <= 5:
            ...
            if self.res == 1:
                self.player+=1
                QMessageBox.about(self, '游戏结束', '玩家获胜!次数为'+self.player)  
            elif self.res == 2:
                self.ai +=1
                QMessageBox.about(self, '游戏结束', '电脑获胜!次数为'+str(self.ai))
            elif self.res == 3:
